...
output "network" {
  description = "Existing network inputs and the standalone external node-pool network attachment."
  value = {
    ...
    attachment_id         = anxcloud_kubernetes_node_pool_network.external.id
    attachment_state      = anxcloud_kubernetes_node_pool_network.external.state
    attachment_state_text = anxcloud_kubernetes_node_pool_network.external.state_text
  }
}
