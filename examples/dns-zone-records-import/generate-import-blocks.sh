...
function run_auto_mode {
  ...
  if ! $TF_CMD plan -no-color >/tmp/plan_output_$$.txt 2>&1; then
  ...
  local plan_output=$(cat /tmp/plan_output_$$.txt)
  rm -f /tmp/plan_output_$$.txt
  ...
  if ! $TF_CMD apply -auto-approve -no-color; then
    echo "❌ Apply failed"
    return 1
  fi
...
EOF
    awk '/^resource "anxcloud_dns_zone"/{flag=1} flag; /^}$/ && flag{flag=0}' _generated.tf >>imported_dns_zone.tf

    # Records file
    cat >imported_dns_records.tf <<EOF
# Imported DNS Records Configuration
# Generated by: generate-import-blocks.sh --auto
# Zone: $zone_name
# Zone names are hardcoded (not using variables).

EOF
    awk '/^resource "anxcloud_dns_record"/{flag=1} flag; /^}$/ && flag{flag=0}' _generated.tf >>imported_dns_records.tf

    # Format
    $TF_CMD fmt imported_dns_zone.tf >/dev/null 2>&1 || true
    $TF_CMD fmt imported_dns_records.tf >/dev/null 2>&1 || true

    rm -f _generated.tf
    echo "  ✅ Created imported_dns_zone.tf"
    echo "  ✅ Created imported_dns_records.tf"
  fi
...
function get_zone_identifier {
  ...
  local response
  response=$(curl -s -H "Authorization: Token $ANEXIA_TOKEN" "$api_url")
...
CONSOLE_EOF
  ...
  cat >>"$OUTPUT_FILE" <<'RECORD_STUB_HEADER'
...
' >>"$OUTPUT_FILE"
  ...
  2. Run: $TF_CMD plan -generate-config-out=_generated.tf
  3. Review _generated.tf and edit as needed
  4. Run: $TF_CMD apply (IMPORT ONLY - no infrastructure modifications)
  5. Clean up when done: rm _temp_*.tf .terraform/ -rf
